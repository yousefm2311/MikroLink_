...
import { protectUser as protect } from "../Middleware/auth.js"; // أو المسار المناسب
import {
  // لو عندك Validators جاهزة للسائق، اعمل نسخ للمستخدم أو استخدم العامة
...
router.post("/signup", signupValidator, async (req, res, next) => {
  try {
    ...
    if (!errors.isEmpty())
      throw new ApiError(400, MESSAGES.system.bad_request, errors.array());
...
router.get("/verify/:token", async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).send("<h2>المستخدم غير موجود</h2>");
...
router.post("/refresh", async (req, res, next) => {
  try {
    ...
    if (user.refreshToken !== refreshToken)
      throw new ApiError(403, MESSAGES.auth.refresh_invalid);

    const newAccessToken = jwt.sign({ id: user._id }, process.env.JWT_SECRET, {
      expiresIn: "15m",
    });
    return ok(res, MESSAGES.auth.refresh_success, {
